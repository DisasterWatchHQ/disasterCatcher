...
const createWarning = async (req, res) => {
  try {
    const { title, disaster_category, description, affected_locations, severity } = req.body;

    if (!title || !disaster_category || !description || !affected_locations || !severity) {
    ...
    const invalidLocations = affected_locations.filter(
      (location) =>
        !location.address?.city || !location.address?.district || !location.address?.province
    );

    if (invalidLocations.length > 0) {
      return res.status(400).json({
        error: "All locations must include address with city, district, and province",
      });
    }
...
const addResponseAction = async (req, res) => {
  try {
    ...
    await createSystemLog(req.user._id, "ADD_WARNING_ACTION", "warning", warning._id, {
...
const updateActionStatus = async (req, res) => {
  try {
    ...
    await createSystemLog(req.user._id, "UPDATE_ACTION_STATUS", "warning", warning._id, {
...
const getWarnings = async (req, res) => {
  try {
    ...
    const warnings = await Warning.find(query)
      ...
      .populate("response_actions.performed_by", "name workId associated_department")
      .sort({ created_at: -1 })
...
const updateWarning = async (req, res) => {
  try {
    ...
    await createSystemLog(req.user._id, "UPDATE_WARNING_STATUS", "warning", warning._id, {
