...
describe("Warning System", () => {
  ...
  describe("Warning Queries", () => {
    test("should get active warnings", async () => {
      const response = await request(app).get("/api/warnings/active");
    ...
    test("should get warnings with filters", async () => {
      const response = await request(app).get("/api/warnings").query({
        disaster_category: "fire",
        severity: "high",
        status: "active",
      });
    ...
    test("should require latitude and longitude for location query", async () => {
      const response = await request(app).get("/api/warnings/location").query({
        latitude: 1.234,
      });
